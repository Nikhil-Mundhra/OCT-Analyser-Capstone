"""
data/preprocessing/multisurface.py

Simultaneous Coupled Multi-Surface Graph Optimization & Continuous B-Spline Regularization Engine.

Key Architectural Components:
1. Hyperreflective RPE Anchor: Locks onto the highest-SNR melanin/Bruch's membrane band.
2. Inverted Bottom-Up ILM Exit Scan: Scans upward from RPE to detect the true retinal tissue exit.
3. Coupled Surface Elasticity DP: Minimizes joint energy with thickness gradient penalty.
4. Continuous Cubic B-Spline Regularization: Enforces C^2 continuity across all surfaces.
"""
...
class JointMultiSurfaceOptimizer:
    ...
    def optimize_surfaces(
        ...
        from data.preprocessing.choroid import detect_rpe_band, compute_sfcm_choroid_boundary
        ...
        for x in range(w):
            ...
            found_y = -1
            for y in range(limit_y, start_y):
                if smoothed[y, x] >= 38.0 and norm_grad[y, x] >= 0.04:
                    below_mean = np.mean(smoothed[y:min(h, y + 15), x])
                    above_mean = np.mean(smoothed[max(0, y - 12):y, x]) if y > 5 else 0.0
                    if below_mean >= 48.0 and above_mean <= 38.0:
                        found_y = y
                        break

            if found_y < 0:
                for y in range(start_y, limit_y, -1):
                    if smoothed[y, x] < 34.0:
                        tissue_above = np.any(smoothed[limit_y:y, x] > 55.0)
                        if not tissue_above:
                            found_y = y
                            break
