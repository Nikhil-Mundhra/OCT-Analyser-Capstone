...
def resolve_and_create_version_dir(base_dir: Path, requested_version: str = "auto", args_dict: Optional[Dict] = None, logger_inst=None) -> Tuple[Path, str]:
    import platform
    import sys
    import subprocess
...
- **Host OS / Arch**: `{platform.system()} {platform.release()} ({platform.machine()})`
...
"""
        ...
        if logger_inst:
            logger_inst.info(f"=== Strict Weight Versioning Active: Initialized {version_dir} with version_metadata.md ===")
...
class MultiHeadTrainer:
    def __init__(
        ...
        if sub_dir:
            base_ckpt_dir = Path(checkpoint_dir) / mode / sub_dir
            base_tb_dir = Path(log_dir) / mode / sub_dir
        else:
            base_ckpt_dir = Path(checkpoint_dir) / mode
            base_tb_dir = Path(log_dir) / mode
            
        if self.compute_manager.is_main_process:
            self.ckpt_dir, self.version_tag = resolve_and_create_version_dir(
                base_dir=base_ckpt_dir,
                requested_version=version,
                args_dict=args_dict,
                logger_inst=logger
            )
            tb_dir = base_tb_dir / self.version_tag
            tb_dir.mkdir(parents=True, exist_ok=True)
            self.writer = SummaryWriter(log_dir=str(tb_dir))
        else:
            # Non-main DDP processes use existing directory
            version_tag = version if version != "auto" else "v1"
            self.ckpt_dir = base_ckpt_dir / version_tag
            self.writer = None
