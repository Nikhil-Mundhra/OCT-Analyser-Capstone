...
The legacy monolithic `preprocessing.py` file was refactored into a modular, single-responsibility Python package under `image-classification-model-training/data/preprocessing/`:

```
image-classification-model-training/data/preprocessing/
├── __init__.py           # Re-exports public API symbols for package access
├── masking.py            # Dynamic dual-pass noise-floor binarization & Gaussian vector smoothing
...
For backwards compatibility, `image-classification-model-training/data/preprocessing.py` remains as a top-level module re-exporting all package functions.
...
Global Otsu thresholding ($\sim 70\text{--}90$) fits bright upper retinal layers well, but prematurely cuts off low-reflectivity choroidal tissue (intensity $30\text{--}50$). The dual-pass engine resolves this using two adaptive passes:
...
Pathological fluid cysts and vessel shadows in RVO, DME, and CNV scans cast wide vertical attenuation beams ($60\text{--}120\text{px}$). The pipeline dynamically computes horizontal closing kernel width based on image resolution:
...
## 🖥️ Interactive Fine-Tuning Dashboard (`scripts/tuning_server.py`)
