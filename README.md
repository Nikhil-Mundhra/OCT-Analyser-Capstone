...
### Dataset Preprocessing & Coupled Multi-Surface Masking Pipeline
- **Simultaneous Coupled Multi-Surface Graph Optimization**: Solves for $\text{ILM}(x)$, $\text{RPE}(x)$, and $\text{Choroid}(x)$ simultaneously in a single joint optimization pass with strict physical inter-layer distance constraints ($\Delta_{\text{min}} \le y_{\text{RPE}} - y_{\text{ILM}} \le \Delta_{\text{max}}$). Completely eliminates error cascading, parameter chasing, and peripheral edge dips.
- **Continuous B-Spline Anatomical Regularization**: Fits cubic B-spline bases with bounded curvature ($\int |S''(x)|^2 dx$), guaranteeing $C^2$ continuity, anti-aliased organic boundaries, and zero high-frequency gradient impulses for deep CNN training.
- **Vitreous-Moat Raycasting & Banner Blanking**: Eliminates top and bottom white header/footer bars of any geometry, thickness, or slant/angle (including diagonal headers like `DME-15307-5.jpeg`, 30%+ thick banners, tapering wedges, or corner text overlays) without touching retinal tissue.
- **Unified Command-Line Interface (CLI)**: Direct single-image and batch-folder processing via `python3 scripts/tuning_server.py --image <path> --save-mask --save-overlay` or `--folder <name> --sample N`.
- **Interactive Fine-Tuning Server**: Local browser dashboard at `http://localhost:8000` for visual calibration and real-time SVG boundary overlay inspection across all 20 dataset subfolders (see [docs/preprocessing_tuning_guide.md](file:///Users/nikhilmundhra/Documents/Github/OCT-Analyser-Capstone/docs/preprocessing_tuning_guide.md) for full guide).
- **Grainy / Sharp Boundary Mitigation for Deep CNN Training**: Hard binary step edges and jagged 1-pixel staircase boundaries act as artificial high-frequency Dirac/step impulses ($\Delta I \gg 0$). Early convolutional stem kernels ($7 \times 7$ ConvNeXt stem or $3 \times 3$ ResNet layers) act as spatial edge detectors that over-specialize on artificial mask borders rather than true retinal biomarkers (e.g. cystoid spaces, drusen, hyperreflective foci), polluting downstream feature maps and distorting Grad-CAM saliency. The pipeline enforces $C^\infty$ continuous B-spline curves and Gaussian smoothing to eliminate artificial step-edge gradient spikes.
- **Deep Learning Segmenter Roadmap (Plan 2)**: Full specification for training an end-to-end zero-parameter U-Net on `OCT5K` ground-truth masks with active pseudo-labeling (see [PLAN_2_NEURAL_SEGMENTATION.md](file:///Users/nikhilmundhra/Documents/Github/OCT-Analyser-Capstone/training/classification/data/preprocessing/PLAN_2_NEURAL_SEGMENTATION.md)).
