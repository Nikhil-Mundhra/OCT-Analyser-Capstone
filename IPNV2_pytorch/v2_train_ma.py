...
def train_net(net,device):
    ...
    for itr in range(0, opt.max_iteration):
        ...
        # train_annotations[train_annotations==255]=2
        # train_annotations = train_annotations.type(torch.int64)
        # print("start reading from here")
        # train_annotations = torch.unsqueeze(train_annotations,dim=0)
        # print(train_annotations.shape)
        # train_annotations = torch.cat((train_annotations, train_annotations),0)
        # print(train_annotations.shape)
        ...
        with torch.no_grad():
            if itr % interval==0:
                ...
                for kk, cube in enumerate(cubelist0):
                    ...
                    label= imageio.imread(os.path.join(opt.dataroot,'val', opt.modality_filename[opt.in_channels], f'{cube}.bmp'))
                    label[label>=1]=1
                    # label[label==255]=2
                    for i in range(0, DATA_SIZE[1],BLOCK_SIZE[1]):
                        for j in range(0, DATA_SIZE[2],BLOCK_SIZE[2] ):
                            ...
                            pred,_ = net(images)
                            pred_argmax = torch.argmax(pred, dim=1) ##### Check why it is always zeros ... 
