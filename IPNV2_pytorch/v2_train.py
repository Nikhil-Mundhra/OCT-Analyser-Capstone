...
'''
# Example usage
# Assuming y_true and y_prob are your true labels and predicted probabilities in NumPy arrays
y_true = np.array([0, 1, 0, 1])
y_prob = np.array([[0.9, 0.1], [0.2, 0.8], [0.8, 0.2], [0.3, 0.7]])

# Compute Brier Score
brier = brier_score(y_true, y_prob[:, 1])  # Assuming y_prob contains probabilities for the positive class
print("Brier Score:", brier)

# Convert probabilities to log probabilities for NLL calculation
y_log_prob = np.log(y_prob)

# Compute Negative Log-Likelihood
nll = negative_log_likelihood(y_true, y_log_prob)
        ...
        with torch.no_grad():
            if itr % interval==0: # every 500 iteration
                ...
                for kk, cube in enumerate(cubelist0):
                    ...
                    logging.info(opt.modality_filename[opt.in_channels])
