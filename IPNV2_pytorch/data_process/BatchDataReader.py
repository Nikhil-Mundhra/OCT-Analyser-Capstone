...
class BatchDatset:
            ...
            for modality in self.filelist.keys():
                if modality != self.modality[-1]: 
                    ...
                    for ct in ctlist:
                        ...
                        self.label[0,:,:,ct_num] = np.array(self.image_transform(labeladress,0,True))
    ...
    def image_transform(self, filename, key, label=False):
        # image = misc.imread(filename)
        image = imageio.imread(filename)/255.0 # division with 255.0 is additional
        if key:
            # resize_image = misc.imresize(image,[self.blocksize[0], self.datasize[1]], interp='nearest')
            resize_image = resize(image, (self.blocksize[0], self.datasize[1]), order=1)
        else:
            resize_image = image
            if label == True:
                logging.info("rotate")
                resize_image = np.rot90(resize_image, 3) # rotate three times = rotate clockwise once
        return np.array(resize_image*255)
    ...
    def read_batch_random_train(self):#vessel segmentation
        for batch in range(0,self.batch_size):
            ...
            self.images[batch, :,:, 0:self.blocksize[1], 0:self.blocksize[2]] = self.data[0,:, startx:endx,starty:endy, ctnum].astype(np.float32)
